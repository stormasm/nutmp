...
/*
    Leave this debug statement here so you can see what is happening,
    prior to the expected_type match
    debug!("Top of parse_arg");
    debug!("expected_type: {:?}",expected_type);
    debug!("lite_arg {:?}",lite_arg);
*/
...
fn parse_internal_command(
    ...
    debug!("Number of args = {}",lite_cmd.args.len());
    ...
    while idx < lite_cmd.args.len() {
        if lite_cmd.args[idx].item.starts_with('-') && lite_cmd.args[idx].item.len() > 1 {
            ...
            debug!("Line 1393 Before parse_arg");
            let (arg, err) = parse_arg(*rest_type, registry, &lite_cmd.args[idx]);
            if error.is_none() {
                error = err;
            }
            debug!("Line 1398 After  parse_arg");
            debug!("{:?}",arg);
            positional.push(arg);
            current_positional += 1;
        } else {
